...
The `build.ninja` file is auto-generated by `configure.py`. When you add or remove
source files (`.cpp`, `.h`) or `CMakeLists.txt` files, run:

```bash
python3 configure.py
```

This updates the build dependencies so ninja knows when to rebuild.
